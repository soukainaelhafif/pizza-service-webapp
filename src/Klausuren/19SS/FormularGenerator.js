...
function init() {
    ...
    for (const element of inputs) {
        element.addEventListener("input", sendData, false);
    }
}

function sendData(e) { // e ist das Event-Objekt, das automatisch übergeben wird
    "use strict";
    let elem = e.target; // wenn die Funktion durch ein Event (onxxx) aufgerufen wurde,
                             // liefert e.target den DOM-Knoten, an dem das Event ausgeloest wurde
    let key = elem.name;
